...
def macro_cluster_update_heatmap_live(n):
    ...
    # var macro_cluster_update_heatmap_live needs to be none as long as there is no data available
    if not hasattr(macro_cluster_update_heatmap_live, "macro_cluster_heatmap_last_figure"):
        macro_cluster_update_heatmap_live.macro_cluster_heatmap_last_figure = None
    ...
    if glob.macro_df:
        try:
            ...
            grouped_df = convert_macro_cluster_visualization(df)
            macro_cluster_heatmap_last_figure = px.bar(
                grouped_df,
                y='macro_cluster',
                x='micro_cluster_tweet_sum',
                title={
            ...
            macro_cluster_heatmap_last_figure.update_traces(marker_color='#07368C')
            macro_cluster_heatmap_last_figure.update_layout(
                plot_bgcolor='rgba(0,0,0,0)',
                paper_bgcolor='rgba(0,0,0,0)',
                yaxis=dict(
                    tickmode='linear',
            ...
            return macro_cluster_heatmap_last_figure

        except Exception as e:
            print(f"An error occurred: {e}")

    # var macro_cluster_update_heatmap_live needs to be none as long as there is no data available
    if macro_cluster_update_heatmap_live.macro_cluster_heatmap_last_figure is None:
        macro_cluster_update_heatmap_live.macro_cluster_heatmap_last_figure = empty_figure

    return macro_cluster_update_heatmap_live.macro_cluster_heatmap_last_figure


# -- MACRO CLUSTER WIDGET HEATMAP --
